...
function yearsOf(value) { return [...new Set((value.match(/(?:19|20)\d{2}/g) ?? []).filter(year => Number(year) <= currentYear))].sort().reverse(); }
...
for (const fullPath of files) {
  ...
  const title = cleanName(fileName); const allText = `${relativePath} ${title} ${contentSample}`; const years = yearsOf(allText); const validFolderYear = h.yearFromFolder && Number(h.yearFromFolder) <= currentYear ? h.yearFromFolder : ''; const year = validFolderYear || years[0] || '未標示年份'; const classification = classify(`${relativePath} ${title}`, contentSample); const contentHash = crypto.createHash('sha256').update(await fs.readFile(fullPath)).digest('hex');
